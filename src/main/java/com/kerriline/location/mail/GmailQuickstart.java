...
public class GmailQuickstart {
    ...
    public static void main(String... args) throws IOException, GeneralSecurityException {
        ...
        ListMessagesResponse listResponse = service.users().messages().list(user).setQ("in:inbox subject:1392 is:unread").execute();
        ...
        if (listResponse.getResultSizeEstimate() == 0) {
            ...
            for (Message msgId : listResponse.getMessages()) {
                System.out.printf("- %s\n", msgId.getId());
                Message message = service.users().messages().get(user, msgId.getId()).execute();
    ...
    public static Message getFullMessage(@NotNull Message message) throws IOException, GeneralSecurityException {
        Gmail service = getGmail();
        String user = "me";
        return service.users().messages().get(user, message.getId()).execute();
    }

    public static String getSubject(@NotNull Message message) {
        List<MessagePartHeader> headers = message.getPayload().getHeaders();
        Optional<String> header = headers.stream().filter(h -> h.getName().equals("Subject")).map(MessagePartHeader::getValue).findFirst();
