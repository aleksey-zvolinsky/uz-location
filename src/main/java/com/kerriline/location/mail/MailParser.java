...
public class MailParser {
    ...
    private final List<String> duplicates = Arrays.asList(
        "СТАНЦИЯ ОПЕРАЦИИ",
        "ДАТА ОПЕРАЦИИ",
        "СТАНЦИЯ НАЗНАЧЕНИЯ",
        "ГРУЗОПОЛУЧАТЕЛЬ",
    ...
    private final List<String> idNameFields = Arrays.asList(
        "КОД ГРУЗА",
    ...
    private static final MapSplitter MAP_SPLITTER = Splitter
        .on('\n')
        .trimResults()
        .omitEmptyStrings()
    ...
    private String prepareText(String body) {
        ...
        while ((newPos = res.indexOf("ПОСЛЕДНИИ ОПЕРАЦИИ", lastPos)) != -1) {
            ...
            b.append(res.substring(newPos, lastPos).replace("\r\n", ";").replace(":", ","));
        }
    ...
    private String processTankNumbers(final String block) {
        ...
        return tank.substring(0, block.indexOf("(")) + block.substring(block.indexOf("\n"));
    }

    /**
     * Add numbers to repeating columns
     *
     * @param block string to process
