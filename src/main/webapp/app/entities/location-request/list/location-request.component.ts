...
export class LocationRequestComponent implements OnInit {
  ...
  protected onSuccess(data: ILocationRequest[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/location-request'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
