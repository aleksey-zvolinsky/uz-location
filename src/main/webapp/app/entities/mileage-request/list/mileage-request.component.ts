...
export class MileageRequestComponent implements OnInit {
  ...
  protected onSuccess(data: IMileageRequest[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/mileage-request'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
