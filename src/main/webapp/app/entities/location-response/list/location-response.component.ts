...
export class LocationResponseComponent implements OnInit {
  ...
  protected onSuccess(data: ILocationResponse[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/location-response'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
